...
int countContains(const double a1[], int n1, const double a2[], int n2)
{
    ...
    if (a2[0] == a1[0]) {
        sequence = countContains(a1 + 1, n1 - 1, a2 + 1, n2 - 1);
        duplicates = countContains(a1 + 1, n1 - 1, a2, n2);
...
void order(double a[], int n)
{
    ...
    order(a, n - 1);    // arrange last element of array in order
    order(a + fl, n - fl);  //  make sure elements after separator are less than the seperator
}
