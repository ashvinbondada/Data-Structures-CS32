...
int main()
{
    ...
    //WORKS: test max capacity returning false in insert -> temporarily make DEFAULT_MAX_ITEMS = 3
        //assert(!a.insert("Anith", 4.000)); <- no longer testable since DEFAULT_MAX_ITEMS = 260 now
    ...
    a.insertOrUpdate("Johnny Appleseed", 2.900);
    a.insertOrUpdate("Johnny Appleseed", 3.011);
    a.get("Johnny Appleseed", w);
    assert(a.contains("Johnny Appleseed") && w == 3.011);
    assert(v == 10 && a.contains("Landon") && a.contains(""));
    
    //test erase function
    a.erase("Johnny Appleseed");
    a.insert("Kyle", 3.879);
    a.erase("Kyle");
    ...
    // test map exclusive tests
    //   map capacity reached
    c.insert("Ashvin", 3.999);
    c.insert("Zion", 3.556);
    c.insert("Bella", 3.987);
    assert(!c.insert("Anith", 4.000));      //   should not insert, because max capacity defined by c is 3
...
Map a(1000);   // a can hold at most 1000 key/value pairs
    ...
    assert(!b.insert(k[5], v[3]));

      // When two Maps' contents are swapped, their capacities are swapped
      // as well:
    a.swap(b);
    assert(!a.insert(k[5], v[3])  &&  b.insert(k[5], v[3]));
