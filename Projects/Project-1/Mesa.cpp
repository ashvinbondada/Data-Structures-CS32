...
bool Mesa::attackGarkAt(int r, int c, int dir)
{
    ...
    bool gark_death = m_garks[k]->getAttacked(dir);
    if (k < m_nGarks  && gark_death)  // gark dies
    {
        delete m_garks[k];
        m_garks[k] = m_garks[m_nGarks-1];
        m_nGarks--;
        m_history.record(m_player->row(), m_player->col());     //  record position of player when attack
        return true;
    }
   else if (k < m_nGarks && !gark_death) {   //  if gark is attacked but doesn't die...
        m_history.record(m_player->row(), m_player->col());     //  record position of player when attack
    }
