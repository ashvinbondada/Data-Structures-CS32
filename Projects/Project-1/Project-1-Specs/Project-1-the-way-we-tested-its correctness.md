# Project 1 - “the way we tested its correctness”

There were 35 test cases; the first five were worth 2 points each, and the rest were worth 3 points each. Each test case is represented by an assertion that must be true for you to pass that test. Each test case was run independently. We ignored any output that was written to the screen (especially by `Mesa::display`) other than the final `Passed` for a passed test. We renamed your main routine to something harmless, replaced the body of the implementation of `clearScreen` with `{}` (an empty function body), and appended this code to your program:
